...
function damageUnit(id, hpDmg, spDmg, reason, sourceId=null, opts={}){
  ...
  const _minHpFloor = (typeof opts.minHpFloor === 'number') ? opts.minHpFloor : null;
...
async function zai_CleavingFrenzy(u, payload){
  ...
  // 1) 刺击：10HP +5SP +1流血
  if(!await hit(10, 5, '刺击', 'RedCut', ()=> addStatusStacks(target, 'bleed', 1, {label:'流血', type:'debuff'}), 150)){
    unitActed(u);
    return;
  }
  // 2) 连斩一：25HP +0SP +1破伤风
  ...
  // 3) 连斩二：15HP +0SP +1流血
  if(!await hit(15, 0, '连斩二', (Math.random()<0.5?'BloodSlash':'RedCut'), ()=> addStatusStacks(target, 'bleed', 1, {label:'流血', type:'debuff'}), 150)){
    unitActed(u);
    return;
  }
  // 4) 连斩三：15HP +0SP +1破伤风
...
async function zai_BloodStar(u){
  ...
  for(let i=1;i<=24;i++){
    ...
    if(i % 3 === 0){ for(const t of getEnemies()) addStatusStacks(t, 'bleed', 1, {label:'流血', type:'debuff'}); }
    ...
    const hitPoint = () => {
      starChain = starChain.then(async () => {
        ...
        for(const t of getEnemies()){
          addStatusStacks(t, 'bleed', 1, {label:'流血', type:'debuff'});
          zaiApplyTetanus(t, 1);
        }
        await sleep(40);
      });
    ...
    const hitFinal = () => {
      starChain = starChain.then(async () => {
        ...
        for(const t of targets){
          ...
          u.dmgDone += actualHp;
          addStatusStacks(t, 'bleed', 5, {label:'流血', type:'debuff'});
          zaiApplyTetanus(t, 1);
          zaiApplyPainAmp(t, 1);
        }
...
function zaiApplyBitePenalty(zai){
  ...
  addStatusStacks(zai, 'bleed', 5, {label:'流血', type:'debuff'});
  renderAll();
}
...
function buildSkillFactoriesForUnit(u){
  ...
  if(u.id==='adora'){
    ...
    if(u._forceBiteThisTurn){
      F.push(
        { key:'暴戾撕咬', prob:1.0, cond:()=>true, make:()=> skill('暴戾撕咬',1,'purple','花费所有剩余步数：以自身为中心3x3撕咬一名敌方单位，100真实伤害，清空其SP并叠5层流血。击杀则自身恢复7%最大生命值并获得5层鸡血。若本回合未咬到任何单位则反噬。满血目标会躲避。',
    ...
    if(!u.oppression){
      F.push(
        { key:'迅捷射击', prob:0.70, cond:()=>true, make:()=> skill('迅捷射击',0,'green','6格内单体 15HP -5SP（未移动伤害+50%；目标<50%双倍）',
...
function processUnitsTurnStart(side){
  ...
  for(const u of sideUnits){
    if(u.spPendingRestore && u.sp<=0){
      const restoreTo = Math.max(1, Math.floor(u.maxSp * u.spPendingRestore));
      u.sp = restoreTo;
    ...
    if(u.status){
      ...
      if(u.status.bleed && u.status.bleed>0){
        ...
        damageUnit(u.id, dmg, 0, `流血：${u.name} 回合开始失血`, null, {trueDamage:true, hideFx:true});
        u.status.bleed -= 1;
    ...
    // Haz bleed (self cost)
    if(u.hazBleedTurns && u.hazBleedTurns>0){
      const dmg = Math.floor(u.maxHp*0.03);
      damageUnit(u.id, dmg, 0, `Haz流血：${u.name} 回合开始失血`, null, {trueDamage:true, hideFx:true});
      u.hazBleedTurns -= 1;
      if(u.hazBleedTurns<=0){ delete u.hazBleedTurns; appendLog(`${u.name} 的Haz流血结束`); }
    }
  }
  ...
  const zai = units['zai'];
  if(zai && zai.hp>0 && zai.side===side && zai.sp <= 40 && !zai._forceBiteThisTurn && !zai._bitePenaltyApplied){
    zai._forceBiteThisTurn = true;
    zai._biteHitThisTurn = false;
    if(side==='enemy') enemySteps += 5; else playerSteps += 5;
    appendLog('吃掉你触发！宰本回合只能使用「暴戾撕咬」并获得额外步数');
  }
